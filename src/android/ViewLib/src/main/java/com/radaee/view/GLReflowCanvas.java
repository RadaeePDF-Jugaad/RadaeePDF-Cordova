...
public class GLReflowCanvas {
    ...
    public GLReflowCanvas(int w, int h, int cell_h, int gap) {
        m_gap = gap;
        m_w = w + gap;
        m_h = h + gap;
        m_cell_h = cell_h;
    ...
    public void layout(Page page) {
        ...
        for (blk_cur = 0; blk_cur < blk_cnt - 1; blk_cur++) {
            m_blks[blk_cur] = new GLReflowBlock(page, y, m_w, m_cell_h, m_gap);
            y += m_cell_h;
        }
        m_blks[blk_cur] = new GLReflowBlock(page, y, m_w, m_h - y, m_gap);//last block
    }
