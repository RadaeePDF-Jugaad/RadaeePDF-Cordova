...
+ (PDF_RECT)getParagraphsToRead:(RDVLayout *)mPdfView withPos:(RDVPos)mPosition forDoc:(PDFDoc *)mDoc withZoom:(BOOL)zoom
{
    PDFPage *mCurrentPage = [mDoc page:mPosition.pageno];
    [mCurrentPage objsStart];
    
    //get the index of the tapped character
    int mClickedCharIndex = [mCurrentPage objsAlignWord:[mCurrentPage objsGetCharIndex:mPosition.pdfx :mPosition.pdfx] :-1];
...
+ (PDF_RECT)toDIBCoordinates:(RDVLayout *)mPdfView withPos:(RDVPos)mPosition para:(PDF_RECT)mPara
{
    PDF_RECT mResult;
    RDVPage *mViewPage = [mPdfView vGetPage:mPosition.pageno];
    int px = [mViewPage GetVX:[mPdfView docx]];
    int py = [mViewPage GetVY:[mPdfView docy]];
    float tmp = mPara.top;
...
+ (void)toDIBCoordinates:(RDVLayout *)mPdfView withPos:(RDVPos)mPosition firstPara:(PDF_RECT)mFirstPara secondPara:(PDF_RECT)mSecondPara {
    RDVPage *mViewPage = [mPdfView vGetPage:mPosition.pageno];
    int px = [mViewPage GetVX:[mPdfView docx]];
    int py = [mViewPage GetVY:[mPdfView docy]];
